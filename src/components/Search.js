...
export default function Search() {
  ...
  useEffect(() => {
    const firstTimeSearch = async () => {
      setProgress(10);
      let apiKey = `https://cors-anywhere.herokuapp.com/https://serpapi.com/search.json?api_key=${process.env.REACT_APP_GOOGLE_API}&engine=google&q=${option}&location=Seattle-Tacoma,+WA,+Washington,+United+States&hl=en&gl=in&google_domain=google.com&num=30&start=10&safe=active`;
      setProgress(30);
          ...
          <div className="col-xl-5 col-lg-5 col-md-7 col-sm-12 col-12">
            ...
            <div className={`${SearchStyle.options} mt-4`}>
              {getDataAPI.search_information.menu_items.map((element)=>{
                return(
              <a  key={element.position}href={element.link} target="_blank" rel="noreferrer" style={{textDecoration:"none"}} ><span className={SearchStyle.topOptions}>{element.title}</span></a> 
                )
              })}
     ...
     {dataFetchByApi?<hr />:""}
      ...
      <div className="container">
        <p className={`${SearchStyle.searchResult} mb-5`}>
          About {getDataAPI.search_information.total_results} results ({getDataAPI.search_information.time_taken_displayed}){" "}
        </p>
        {getDataAPI.organic_results.map((ele) => {
          return (
        
        <div className="row mx-xl-5 mx-lg-5 mx-md-2 mx-sm-0 mx-0 mt-3" key={ele.position}>
          <div className="col-xl-8 col-lg-8 co-md-10 col-sm-12 col-12">
            <div style={{ display: "flex" }}>
              <img src={ele.favicon} alt="icon" className={SearchStyle.webIconStyle} />
              <div style={{ display: "grid" }}>
                ...
                <small
                  ...
                  style={{ fontSize: "11px" }}
                >
                  {ele.displayed_link}{" "}<i className={`fa-solid fa-ellipsis-vertical ${SearchStyle.iconStyleThreeDots}`} style={{marginLeft:"30px",fontSize:"15px"}}></i>
                </small>
              </div>
            </div>
            <a href={ele.link} target="_blank" rel="noreferrer"><strong className="mt-3" style={{ color: "rgb(138,180,248)" }}>
              {ele.title}
            </strong></a>
